...
import { getChildById } from 'apiSdk/children';
import { ChildInterface } from 'interfaces/child';
import { PostListPage } from 'pages/posts';
import { SimulatorProfileListPage } from 'pages/simulator-profiles';

function ChildViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ChildInterface>(
    () => (id ? `/children/${id}` : null),
    () =>
      getChildById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('child', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/children/edit/${id}`} passHref legacyBehavior>
                    <Button
