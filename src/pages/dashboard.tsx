...
import { getSimulatorProfiles } from 'apiSdk/simulator-profiles';
...
import { SimulatorProfileListPage as ShowCaseList } from 'pages/simulator-profiles';

function DashboardPage() {
  const { session } = useSession();

  const simulatorProfileStat = useSWR<PaginatedInterface<SimulatorProfileInterface>>(
    () => `/simulator-profiles`,
    () => getSimulatorProfiles({ limit: 1, offset: 0 }),
  );

  const childStat = useSWR<PaginatedInterface<ChildInterface>>(
    () => `/children`,
    () => getChildren({ limit: 1, offset: 0 }),
  );

  const postStat = useSWR<PaginatedInterface<PostInterface>>(
    () => `/posts`,
    () => getPosts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Simulator Profiles',
      result: simulatorProfileStat?.data?.totalCount,
      isLoading: simulatorProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Children',
      result: childStat?.data?.totalCount,
      isLoading: childStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Posts',
      result: postStat?.data?.totalCount,
      isLoading: postStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Simulator Profiles
                </Text>
                <NextLink href={`simulator-profiles`} passHref legacyBehavior>
                  <Button
