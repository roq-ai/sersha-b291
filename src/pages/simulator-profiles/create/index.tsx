...
import { createSimulatorProfile } from 'apiSdk/simulator-profiles';
...
function SimulatorProfileCreatePage() {
  ...
  const handleSubmit = async (values: SimulatorProfileInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createSimulatorProfile(values);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Create Simulator Profile',
              isCurrent: true,
            },
          ]}
        />
